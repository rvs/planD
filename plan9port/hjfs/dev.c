...
devwork(void *v)
{
	...
	for(;;){
		...
		b->wnext = b->wprev = nil;
		qunlock(&d->workl);
		if(b->BufReq9.d == nil) /* this is a sync request */
			goto reply;
		if(b->BufReq9.off >= d->size){
			b->error = Einval;
			goto reply;
		}
		b->error = nil;
		if(b->op & BWRITE){
			...
			pack(b, buf);
			if(pwrite(d->fd, buf, BLOCK, b->BufReq9.off*BLOCK) < BLOCK){
				dprint("write: %r\n");
				b->error = Eio;
			}
		}else{
			...
			for(n = 0; n < BLOCK; n += m){
				m = pread(d->fd, buf+n, BLOCK-n, b->BufReq9.off*BLOCK+n);
				if(m < 0)
					dprint("read: %r\n");
				if(m <= 0)
					break;
