...
changedev(Buf *b, Dev *d, uvlong off)
{
	...
	b->BufReq9.d = d;
	if(d != nil){
		b->dnext = &d->buf[b->BufReq9.off & BUFHASH];
...
again:
	if(req.d == b->BufReq9.d && req.off == b->BufReq9.off){
		markbusy(b);
		send(req.resp, &b);
		return;
	}
	...
	for(c = l->dnext; c != l; c = c->dnext)
		if(c->BufReq9.off == req.off){
			if(c->busy)
				delayreq(req, &c->BufReq9.next, &c->last);
			else{
				markbusy(c);
				send(req.resp, &c);
			}
			if(b->BufReq9.next == nil)
				return;
			req = undelayreq(&b->BufReq9.next, &b->last);
			goto again;
		}
	if(b->BufReq9.next != nil){
		givebuf(undelayreq(&b->BufReq9.next, &b->last), b);
		b = bfree.fnext;
	}
	if(b == &bfree){
	...
	markbusy(b);
	if(b->BufReq9.d != nil && b->op & BDELWRI){
		delayreq(req, &b->BufReq9.next, &b->last);
		b->op &= ~BDELWRI;
		b->op |= BWRITE;
		b->BufReq9.resp = putb;
		work(b->BufReq9.d, b);
