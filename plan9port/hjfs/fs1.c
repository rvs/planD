...
getdent(FLoc *l, Buf *b)
{
	...
	if((d->mode & (DGONE | DALLOC)) == 0){
		dprint("getdent: file gone, d=%llux, l=%llud/%d %llux, callerpc %#p\n",
			d->Qid9.path, l->blk, l->deind, l->Qid9.path, getcallerpc(&l));
		werrstr("phase error -- directory entry for nonexistent file");
		return nil;
	}
	if(qidcmp(&d->Qid9, &l->Qid9) != 0){
...
getloc(Fs *fs, FLoc f, Loc *next)
{
	...
	if(next != nil && next->child != nil){
		l = next->child;
		do{
			if(l->FLoc9.blk == f.blk && l->FLoc9.deind == f.deind){
				l->ref++;
				qunlock(&fs->loctree);
				return l;
			}
...
haveloc(Fs *fs, uvlong blk, int deind, Loc *next)
{
	...
	l = next->child;
	if(l != nil) do{
		if(l->FLoc9.blk == blk && l->FLoc9.deind == deind){
			qunlock(&fs->loctree);
			return 1;
		}
		l = l->cnext;
...
freeit:
		if((l->flags & LGONE) != 0){
			 ...
			 */
			qunlock(&fs->loctree);
			b = getbuf(fs->d, l->FLoc9.blk, TDENTRY, 0);
			if(b != nil){
				delete(fs, &l->FLoc9, b);
				putbuf(b);
			}
...
static int
dumpblk(Fs *fs, FLoc *dummy, uvlong *l)
{
	uvlong n;
	int i;
	Buf *b, *c;
 ...
 */
int
trunc9(Fs *fs, FLoc *ll, Buf *bd, uvlong size)
{
	uvlong blk;
	Dentry *d;
	int a[NINDIRECT];
		...
		for(j = 0; j < DEPERBLK; j++)
			if((c->de[j].mode & DALLOC) != 0 &&
			   (name == nil || strcmp(c->de[j].name, name) == 0)){
				if(dump && (c->de[j].Qid9.type & QTTMP) != 0)
					continue;
			   	if(rl != nil){
					rl->Qid9 = c->de[j].Qid9;
			   		rl->blk = r;
...
modified(Chan *ch, Dentry *d)
{
	...
	d->atime = d->mtime;
	d->muid = ch->uid;
	ch->loc->FLoc9.Qid9.vers = ++d->Qid9.vers;
}
...
deltraverse(Fs *fs, Del *p, Buf *b, Del **last)
{
	...
	frb = b == nil;
	if(frb){
		b = getbuf(fs->d, p->FLoc9.blk, TDENTRY, 0);
		if(b == nil)
			return -1;
	}
	d = getdent(&p->FLoc9, b);
	...
	s = d->size;
	for(i = 0; i < s; i++){
		rc = getblk(fs, &p->FLoc9, b, i, &r, GBREAD);
		if(rc <= 0)
			continue;
		c = getbuf(fs->d, r, TDENTRY, 0);
		if(c == nil)
...
delete(Fs *fs, FLoc *l, Buf *b)
{
	...
	for(p = last; p != nil; q = p->prev, free(p), p = q){
		if(p == first)
			...
			c = getbuf(fs->d, p->FLoc9.blk, TDENTRY, 0);
		if(c == nil)
			continue;
		d = getdent(&p->FLoc9, c);
		if(d != nil){
			trunc9(fs, &p->FLoc9, c, 0);
			memset(d, 0, sizeof(*d));
			c->op |= BDELWRI;
		}
		if(p != first)
...
newentry(Fs *fs, Loc *l, Buf *b, char *name, FLoc *res, int dump)
{
	...
	si = sj = -1;
	for(i = 0; i < d->size; i++){
		if(getblk(fs, &l->FLoc9, b, i, &r, GBREAD) <= 0)
			continue;
		c = getbuf(fs->d, r, TDENTRY, 0);
		if(c == nil)
			continue;
	...
	if(si == -1 && i == d->size){
		if(getblk(fs, &l->FLoc9, b, i, &r, GBCREATE) >= 0){
			c = getbuf(fs->d, r, TDENTRY, 1);
			if(c != nil){
				si = i;
				sj = 0;
	...
	if(getblk(fs, &l->FLoc9, b, si, &res->blk, dump != 0 ? GBWRITE : GBREAD) <= 0)
		return -1;
	res->deind = sj;
	res->Qid9 = (Qid){0, 0, 0};
	return 1;
